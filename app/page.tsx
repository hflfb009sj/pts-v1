...
<div className="text-2xl font-black text-amber-500 tracking-widest mb-2">{escrowResult.escrowCode}</div>
        ...
        {tab === 'seller' && (
          <div className="space-y-5">
            <div className="bg-neutral-900/60 border border-white/10 rounded-3xl p-6 backdrop-blur-2xl">
              <h2 className="text-xl font-black italic mb-5 flex items-center gap-2"><Package className="text-amber-500" size={20} /> Seller Dashboard</h2>
              {!sellerTx ? (
                ...
                <div className="space-y-4">
                  <div className="bg-black/30 rounded-2xl p-4 space-y-3">
                    ...
                    {sellerTx.deliveryDeadline && sellerTx.status === 'ACCEPTED' && (
                      <div className="text-[10px] text-amber-500/70">Delivery deadline: {new Date(sellerTx.deliveryDeadline).toLocaleDateString()}</div>
                    )}
                  </div>
{sellerTx.status === 'PENDING' && (
                  ...
                  {sellerError && <div className="text-red-400 text-xs p-3 bg-red-500/5 rounded-xl border border-red-500/20">{sellerError}</div>}
                  <button onClick={() => { setSellerTx(null); setSellerCode(''); }} className="w-full py-3 text-neutral-500 text-sm font-black hover:text-white transition-all">Look up another escrow</button>
                </div>
              )}
            </div>
          </div>
