  ...
  try {
    ...
    if (action === "getAll") {
      ...
      const stats = {
        ...
        frozen:    await db.collection("transactions").countDocuments({ status: { $in: ["FROZEN", "UNDER_REVIEW"] } }),
    ...
    if (action === "refund") {
      ...
      await db.collection("transactions").updateOne(
        { escrowCode: escrowCode.toUpperCase() },
        {
          ...
          $push: { auditLog: { action: "ADMIN_REFUND", by: ADMIN, at: now, note: reason || "Emergency refund" } } as any,
        }
      );
    ...
    if (action === "freeze") {
      await db.collection("transactions").updateOne(
        { escrowCode: escrowCode.toUpperCase() },
        {
          ...
          $push: { auditLog: { action: "ADMIN_FREEZE", by: ADMIN, at: now, note: reason || "Frozen by admin" } } as any,
        }
      );
    ...
    if (action === "resolve") {
      ...
      await db.collection("transactions").updateOne(
        { escrowCode: escrowCode.toUpperCase() },
        {
          ...
          $push: { auditLog: { action: "ADMIN_RESOLVE", by: ADMIN, at: now, note: "Resolved for " + resolveFor } } as any,
        }
      );
